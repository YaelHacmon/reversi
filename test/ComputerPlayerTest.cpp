 ...
 */
ComputerPlayerTest::ComputerPlayerTest(): hp1("X", Board::BLACK), hp2("O",Board::WHITE), hp3("O",Board::WHITE),
		opponent("X", Board::BLACK), player("O",Board::WHITE), tieBoard(4), firstMove(-1,-1), betterMove(-1,-1),
...
void ComputerPlayerTest::setWhiteMoves(vector<Location>& locs, Board& b, Player& p) {
	for (int i = 0; i<locs.size(); i++) {
		b.makeInColor(Board::WHITE, locs[i]);
		p.increaseScore(1);
	}
}
...
//sets white moves by given locations
//accepts the player in case that in the future we would want the opponent white and the computer black
void ComputerPlayerTest::setBlackMoves(vector<Location>& locs, Board& b, Player& p) {
	for (int i = 0; i<locs.size(); i++) {
		b.makeInColor(Board::BLACK, locs[i]);
		p.increaseScore(1);
	}
}
