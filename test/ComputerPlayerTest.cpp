...
hp1("X", Board::BLACK), hp2("O", Board::WHITE), hp3("O", Board::WHITE), hp1Clone(
		NULL), hp2Clone(NULL), hp3Clone(NULL), player("O", Board::WHITE), opponent(
				"X", Board::BLACK), tieBoard(4), firstMove(-1, -1), betterMove(-1, -1), moveSeriesMove(
...
void ComputerPlayerTest::setFirstBoard() {
	...
	//get move - should be (2,2) or (4,2) - by our example
	//our algorithm returns first move - (2,2)
...
void ComputerPlayerTest::setBetterBoard() {
	...
	//create line - play (4,5) to flip (4,4)
	vector < Location > locs;
	...
	//our algorithm takes first move seen - (1,5)
	mvl.updateMoveOptions(&player, betterBoard);
	betterMove = player.getNextMove(&v, &mvl, betterBoard, &opponent);
}

//set board to a series of moves
void ComputerPlayerTest::setMoveSeriesBoard() {
	//clear player's scores to be 2 and 2
	clearScores();

	vector < Location > locs;
	locs.push_back(Location(2,3));
	locs.push_back(Location(2,2));
	locs.push_back(Location(2,1));
	locs.push_back(Location(1,1));
	locs.push_back(Location(0,1));
	locs.push_back(Location(0,0));
	locs.push_back(Location(3,2));
	locs.push_back(Location(2,4));
	locs.push_back(Location(1,2));
	locs.push_back(Location(4,2));
	locs.push_back(Location(1,0));
	locs.push_back(Location(0,2));
	locs.push_back(Location(0,3));
	playVector(locs, moveSeriesBoard);

	//get computer's move - should be (3,1)
	mvl.updateMoveOptions(&player, moveSeriesBoard);
	moveSeriesMove = player.getNextMove(&v, &mvl, moveSeriesBoard, &opponent);
}
...
void ComputerPlayerTest::setWinBoard() {
	...
	//get computer's move - should be (1,5) and NOT (5,1) which is "better scoring" by 1, but will allow game to continue
...
void ComputerPlayerTest::setMaxBoard() {
	...
	//add white moves
	vector < Location > locs;
	locs.push_back(Location(0,0)); //will not be played
	locs.push_back(Location(3, 5));
	playVector(locs, maxBoard);
	//add black move
	locs.clear();
	locs.push_back(Location(3,6));
	setBlackMoves(locs, maxBoard, opponent);

	//get computer's move - should be (3,7), which has the lowest score
